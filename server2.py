...
class Server:
	...
	def messagechannel(self, client, sentLine):
		try:
			#Sets up variables
			targetChannel = sentLine[1]
			message = sentLine[1:]
			msg=""
			targetChan = {}
			...
			#Checks for error codes
			anyError = self.handleMessages(targetChannel, msg)

			#If the channel name is good
			if anyError == "NO_ERROR":
				if targetChan:
					self.sendMessage(targetChan, msg, False)
			else:
				self.messageErrors(anyError, client, targetChannel, "NOTICE")	

		except:
			self.printToServer(client, "Not enough params")
			anyError = self.handleMessages("", "")
			self.messageErrors(anyError, client, "", "NOTICE")		
	...
	def checkCommands(self, command, client):
		...
		for lines in command:
			...
			#Command used to quit server
				...
				self.messagechannel(client, sentLine)
